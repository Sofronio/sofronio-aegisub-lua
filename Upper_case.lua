script_name="Upper Case"
script_description="Capitalises text in the selected line"
script_author="Sofronio moded unanimated"
script_version="1.0"
script_namespace="ua.UpperCase"
...
function case(subs,sel)
    for z,i in ipairs(sel) do
		...
		if true then
			...
			t=sentences(t)
			if subs[i-1].text~=nil then
				if string.sub(t, 1, 2) ~= "I " then
					if string.sub(subs[i-1].text, -1) == "," then
						t=ulower(string.sub(t, 1, 1)) .. string.sub(t, 2, -1)
					end
				end
			end
		end
		if P=="Words" then
			if not true then t=lowercase(t) end
			t=capitalise(t)
		end
		line.text=t
		subs[i]=line
...
function lowercase(t)
	...
	:gsub("\\[Nnh]","{%1}")
	:gsub("^([^{]*)",function(l)
		if true then l=re.sub(l,[[\b(\u\u+'?\u*)]],function(u) return ulower(u) end) return l
...
somewords={"English","Japanese","American","British","German","French","Spanish","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday","January","February","April","June","July","August","September","October","November","December","SPG"}
